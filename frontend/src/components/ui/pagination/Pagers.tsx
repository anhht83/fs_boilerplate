...
const NextMore = (props: any) => {
  const { className, onArrow } = props;
...
const Pagers = (props: any) => {
  ...
  const onPagerClick = (value: any, e: any) => {
    e.preventDefault();
    let newPage = value;
  ...
  const getPagerClass = (index: any) => {
    return `${pagerClass.default} ${currentPage === index ? pagerClass.active : pagerClass.inactive}`;
  };
  ...
  return (
    <ul>
      ...
      {showPrevMore && (
        <PrevMore
          onArrow={(arrow: any) => onArrowClick(arrow)}
          className={`${pagerClass.default} ${pagerClass.inactive}`}
        />
      )}
